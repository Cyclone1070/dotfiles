...
const Color = "#4f88ff"; // If set to "default," it will use the theme's cursor color.
...
function createTrail(options) {
    ...
    function calculatePosition() {
        ...
        for (const particleIndex in particles) {
            const nextParticlePos = (
                particles[+particleIndex + 1] || particles[0]
            ).position;
...
async function createCursorHandler(handlerFunctions) {
    ...
    function createCursorUpdateHandler(
        ...
        let update = (editorX, editorY) => {
            ...
            if (revX == lastX && revY == lastY && lastCursor == cursorId)
                return;
            lastX = revX;
