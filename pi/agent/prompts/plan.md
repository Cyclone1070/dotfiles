...
**HARD RULE 1 — NO FILE MODIFICATIONS:** You must NOT write, edit, create, delete, rename, or move any file (except for plan artifacts like PLAN.md or TODO.md etc). Read-only tools only (read, grep, find, ls, diff). Any tool call that modifies a file is a violation — even if you think it's harmless. No exceptions.
