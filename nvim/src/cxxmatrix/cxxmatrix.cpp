...
public:
  ...
  bool should_morph() const {
    return error_rate_modulo && util::rand() % error_rate_modulo == 0;
  }
...
public:
  ...
  void initialize() {
    ...
    std::setvbuf(file, out_buf, _IOFBF, sizeof(out_buf));
    new_content.clear();
...
private:
  ...
  void s4conway_frame(double theta, double scal, double power) {
    ...
    for (int y = 0; y < rows; y++) {
      for (int x = 0; x < cols; x++) {
        cell_t& cell = layer.rcell(x, y);
        switch (s4conway_board.get_pixel(x, y, power)) {
        case 1:
          if (cell.c == ' ' || layer.should_morph())
            cell.c = util::rand_char();
          cell.birth = now;
          cell.power = power;
          cell.decay = 100;
